...
//! - `organizer_id` is immutable after creation (DB FK `ON DELETE RESTRICT`).
...
/// reference an optional cabinet and a required organizer (teacher/admin).
/// Students participate via `EventAttendee` rows in `event_attendees`.
...
pub struct Event {
    ...
    /// Organizer/leader user ID. Immutable after creation (FK `ON DELETE RESTRICT`).
...
/// Representation of a student's participation in an event.
///
/// DB: `event_attendees` table with a UNIQUE index on `(event_id, student_id)`
/// — a student cannot attend the same event twice (idempotent by design).
...
pub struct EventAttendee {
    ...
    pub event_id: Uuid,
    /// The attending student (FK `ON DELETE CASCADE`).
    pub student_id: Uuid,
...
impl EventAttendee {
    ...
    /// referential integrity (FKs) and the "one student per event once"
