...
-- - is_override: TRUE — replacement template (teacher is sick, cabinet replacement)
...
(
    ...
    -- Whether this is a replacement (TRUE) or a regular template (FALSE)
    is_override BOOLEAN     NOT NULL DEFAULT FALSE,

    -- Comment (for replacements: "Иванов заболел, замена Петровым" — Ivanov is sick, replaced by Petrov)
    comment     TEXT NULL,
...
-- Lessons (only if not overridden by events)
...
WHERE li.lesson_date = p_date
  ...
  -- Exclude lessons overridden by events
